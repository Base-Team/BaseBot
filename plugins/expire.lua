local function check_member_superrem2(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  send_large_msg(channel, "<code> به دلیل تنظیم نکردن زمان گروه ربات از گروه لفت میدهد !</code>")
...
local function pre_process(msg)
	...
	if expiretime then    
		...
		rem_mutes(msg.to.id)
		superrem2(msg)
		return send_large_msg(get_receiver(msg), 'تاریخ اتقضاء گروه به پایان رسید.\n از پشتیبانی در خواست تمدید کنید.[ @PvSuDo ] \n اگر ریپورت هستید به ربات ما مراجعه کنید [ @PvSuDo_Bot ]')
		else
			return
		end
	end
...
local exppm = '<code>🔊 شارژ گروه به پایان رسید </code>\n'
..'----------------------------------\n'
...
..'@Mrr619'
   ...
   redis:hset('expires0',msg.to.id,'0')
	...
	if tonumber(timetoexpire) == 1 then
      ...
      local user = "user#id"..219201071
			local text2 = "(1) روز تا پایان تاریخ انقضاء گروه باقی مانده است\nنسبت به تمدید اقدام کنید.[ @PvSuDo ] \n اگر ریپورت هستید به ربات ما مراجعه کنید [ @PvDuDo_Bot ]"
			local text13 = "1"
			local data = load_data(_config.moderation.data)
			local group_owner = data[tostring(msg.to.id)]['set_owner']
			if not group_owner then
...
function run(msg, matches)
	...
	if matches[1]:lower() == 'expire' then
		local expiretime = redis:hget ('expiretime', get_receiver(msg))
		if not expiretime then return '<code> 》》 متاسفانه تاربخ انقضا گروه هنوز تنظیم نشده است !《《' else
			local now = tonumber(os.time())
...
local text3 = "<code> 》》درخواست شارژ گروه توسط صاحب گروه ارسال شده است《《 </code>"
